...
export function stderr( ...message: MessageArgument[] ): Promise<void>;
export function stderr( mute: boolean, ...message: MessageArgument[] ): Promise<void|Buffer>;

export async function stderr( ...params: params ): Promise<void|Buffer> {
  ...
  if( params.length >= 2 ){
    ...
    if( mute === false ){
      ...
      process.stderr.write( messaging( message_concat ) );
    } else {
